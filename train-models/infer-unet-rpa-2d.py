...
def channels2masknumbers(mask):
    out = torch.zeros(size=mask.shape)
    out = out.to(torch.uint8)
    out = out.to(device)
    for levelnum in num2levelname.keys():
        out += (out == 0).to(torch.uint8) * mask[levelnum-1,...].to(torch.uint8) * levelnum
    return out.to(torch.uint8)
...
class RpaDatasetForInference(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #slice_ = np.random.randint(low=0, high=img.shape[2])
        #imgslice = F.interpolate(img[..., slice_].unsqueeze(dim=0).unsqueeze(dim=0), size=(1024, 1024)).squeeze(dim=(0))
        #binmask = F.interpolate(mask[..., slice_].unsqueeze(dim=0).unsqueeze(dim=0), size=(len(num2levelname), 1024, 1024)).squeeze(dim=(0,1))
    ...
    def postprocess(self, prediction, metadata, target_level='all'):
        if target_level != 'all':
            prediction = (F.sigmoid(prediction) > 0.5).to(torch.uint8) 
        else:
            prediction = F.softmax(prediction, dim=1)
            prediction = torch.argmax(prediction, dim=1).to(torch.uint8).unsqueeze(dim=1)
        return F.interpolate(prediction.unsqueeze(dim=0), size=(metadata['shape']['mask'])).squeeze(dim=(0,1))
...
def main():
    ...
    print(f"Work dir: {train_dataset.output_path}")
    ...
    with torch.no_grad():
        for i in tqdm(range(len(train_dataset))):
            ...
            unet_pred = torch.zeros(size=(1, n_out_channels, *gt2D.shape[-3:]), dtype=torch.float)
            unet_pred.to(device)
            for slice_ in range(image.shape[-1]):
                unet_pred[..., slice_] = unet_model(image[..., slice_].unsqueeze(dim=0)).squeeze(dim=(0,1))
            mask = train_dataset.postprocess(unet_pred[0,...], metadata, target_level=target_level)
